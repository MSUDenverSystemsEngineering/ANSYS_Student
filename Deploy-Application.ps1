...
Try {
	...
	[string]$appVendor = 'ANSYS For Students'
	[string]$appName = ''
	[string]$appVersion = '19.2'
	[string]$appArch = 'x64'
	[string]$appLang = 'EN'
	...
	If ($deploymentType -ine 'Uninstall') {
		...
		$exitCode = Execute-Process -Path "$dirFiles\setup.exe" -Parameters "-silent -disablerss -nohelp" -PassThru
